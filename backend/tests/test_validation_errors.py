...
async def test_validation_error_does_not_echo_sensitive_input() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(
            ...
            headers={"X-Correlation-ID": "validation-test-correlation"},
        )
    ...
    assert body["correlation_id"] == "validation-test-correlation"
    assert response.headers["X-Correlation-ID"] == "validation-test-correlation"
    assert {
