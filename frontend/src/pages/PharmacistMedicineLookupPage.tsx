    ...
    <div className="min-h-screen bg-[#0b1730] text-slate-100 lg:grid lg:grid-cols-[310px_1fr]">
      ...
      <div className="min-w-0">
        <header className="flex min-h-[84px] items-center justify-between border-b border-slate-800 bg-[#0d152a] px-5 sm:px-8">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-400">UC007</p>
            <h1 className="mt-1 text-xl font-bold">Tra cứu thuốc</h1>
          </div>
        ...
        <main className="p-5 sm:p-8 lg:p-10">
          <section className="rounded-3xl border border-slate-700/70 bg-[#18253a] p-6 sm:p-7">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
                  Tìm theo mã hoặc tên thuốc. Dược sĩ chỉ được xem dữ liệu tại màn hình này; thao tác thêm, sửa và xóa danh mục vẫn thuộc Quản lý.
                </p>
              </div>
              <div className="w-full lg:max-w-md">
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500" htmlFor="medicine-search">
                  Từ khóa
                </label>
                <input
                  id="medicine-search"
                  ...
                  placeholder="Ví dụ: PAR-001 hoặc Paracetamol"
              ...
              {medicines.isPending ? (
                ...
                <div className="divide-y divide-slate-700/60">
                  {medicines.data.map((medicine) => (
                    <div
                      key={medicine.id}
                      className="grid grid-cols-[0.8fr_1.4fr_1fr_0.8fr] gap-3 px-4 py-4 text-sm"
                    >
            ...
            <div className="mt-5 rounded-2xl border border-emerald-700/30 bg-emerald-950/20 px-4 py-3 text-xs leading-6 text-emerald-200/80">
              Đây là dữ liệu nghiệp vụ từ PostgreSQL. Trang không có nút chỉnh sửa và backend chỉ cho phép MANAGER/PHARMACIST gọi endpoint tra cứu.
            </div>
          </section>
        </main>
      </div>
